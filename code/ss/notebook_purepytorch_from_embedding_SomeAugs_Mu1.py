...
from models_mg import Simple50Embedding, Simple50Cls
...
emd = Simple50Embedding(embedding_dimension=512)
load = torch.load('./history/notebook_purepytorch_SomeAugs_arcface_embedding_36.pth')
emd.load_state_dict(load['embedder'])
classifier = Simple50Cls(emd, embedding_dimension=512).cuda()

optimizer = Over9000(
    [
        {'params': classifier.embedding.parameters(), 'lr': .002},
        {'params': classifier.neck.parameters(), 'lr': .01}, 
        {'params': classifier.cls_g.parameters(), 'lr': .01}, 
        {'params': classifier.cls_v.parameters(), 'lr': .01},
        {'params': classifier.cls_c.parameters(), 'lr': .01},
    ],
    lr=.001,
)
...
for i in range(N_EPOCHS):
    ...
    for j, (trn_imgs_batch, trn_lbls_batch) in enumerate(training_loader):
        ...
        # mixup
        trn_imgs_batch_device_mixup, trn_lbls_batch_device_shfl, gamma = mixup(trn_imgs_batch_device, trn_lbls_batch_device, .4)
