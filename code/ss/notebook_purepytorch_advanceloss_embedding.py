...
from models_mg import SERESXT50MishGeM_Embedding
from mixup_pytorch_utils import mixup, mixup_loss
from loss import QAMFace
import utils
...
checkpoint_name = 'notebook_purepytorch_advanceloss_embedding_{:d}.pth'


# In[8]:


classifier = SERESXT50MishGeM_Embedding().cuda()
qam_face_g = QAMFace(256, 168).cuda()
qam_face_v = QAMFace(256, 11).cuda()
qam_face_c = QAMFace(256, 7).cuda()

# optimizer = Over9000(classifier.parameters(), lr=.01)
optimizer = Over9000(
    [
        {'params': classifier.parameters()}, 
        {'params': qam_face_g.parameters(), 'weight_decay': .0005},
        {'params': qam_face_v.parameters(), 'weight_decay': .0005},
        {'params': qam_face_c.parameters(), 'weight_decay': .0005},
    ],
    lr=.01,
)
...
for i in range(N_EPOCHS):
    ...
    for j, (trn_imgs_batch, trn_lbls_batch) in enumerate(training_loader):
        ...
        qloss_g = qam_face_g(logits_g, trn_lbls_batch_device[:, 0])
        qloss_v = qam_face_v(logits_v, trn_lbls_batch_device[:, 1])
        qloss_c = qam_face_c(logits_c, trn_lbls_batch_device[:, 2])
        
        loss_g = F.cross_entropy(qloss_g, trn_lbls_batch_device[:, 0], reduction=reduction)
        loss_v = F.cross_entropy(qloss_v, trn_lbls_batch_device[:, 1], reduction=reduction)
        loss_c = F.cross_entropy(qloss_c, trn_lbls_batch_device[:, 2], reduction=reduction)
        ...
        clip_grad_value_(qam_face_g.parameters(), 1.0)
        clip_grad_value_(qam_face_v.parameters(), 1.0)
        clip_grad_value_(qam_face_c.parameters(), 1.0)
    ...
    with torch.no_grad():
        for k, (vld_imgs_batch, vld_lbls_batch) in enumerate(validation_loader):
            ...
            # loss
            qloss_g = qam_face_g(logits_g, vld_lbls_batch_device[:, 0])
            qloss_v = qam_face_v(logits_v, vld_lbls_batch_device[:, 1])
            qloss_c = qam_face_c(logits_c, vld_lbls_batch_device[:, 2])

            loss_g = F.cross_entropy(qloss_g, vld_lbls_batch_device[:, 0], reduction=reduction)
            loss_v = F.cross_entropy(qloss_v, vld_lbls_batch_device[:, 1], reduction=reduction)
            loss_c = F.cross_entropy(qloss_c, vld_lbls_batch_device[:, 2], reduction=reduction)
