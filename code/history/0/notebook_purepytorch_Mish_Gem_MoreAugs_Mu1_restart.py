...
checkpoint_name = 'notebook_purepytorch_Mish_Gem_MoreAugs_Mu1_restart_epoch{:d}.pth'


# In[8]:


classifier = Seresnext50MishGeM().cuda()
...
for i in range(N_EPOCHS):
    ...
    for j, (trn_imgs_batch, trn_lbls_batch) in enumerate(training_loader):
        ...
        total_trn_loss = .6*loss_g + .2*loss_v + .2*loss_c
        ...
        utils.display_progress(len(training_loader), j+1, {'training_loss': epoch_trn_loss[-1], 'current_lr': cur_lr[0]})
    ...
    with torch.no_grad():
        for k, (vld_imgs_batch, vld_lbls_batch) in enumerate(validation_loader):
            ...
            total_vld_loss = .6*loss_g + .2*loss_v + .2*loss_c
            # record
