...
from models_mg import mdl_sext50_AdvLss
# from mixup_pytorch_utils import mixup, mixup_loss
from loss import CenterLoss, AngularPenaltySMLoss, QAMFace, ArcMarginProduct
import utils
...
augs = iaa.SomeOf(
    ...
    [
        ...
        iaa.DirectedEdgeDetect(alpha=(.1, .9), direction=(0.0, 1.0)),
        #iaa.Cutout(nb_iterations=1, size=(.1, .25), squared=False),
    ],
    random_order=True,
)
...
for fold, (trn_ndx, vld_ndx) in enumerate(skf.split(pdf['image_id'].values.reshape(-1, 1), pdf.loc[:, ['grapheme_root', 'vowel_diacritic', 'consonant_diacritic', 'grapheme_code']].values)):
    if fold == 0:
        break
...
checkpoint_name = 'pytorch_model_test_epoch.pth'

# =========================================================================================
# n_grapheme=168, n_vowel=11, n_consonant=7
classifier = mdl_sext50_AdvLss().cuda()
feat_loss = ArcMarginProduct(512, 1295).cuda()

optimizer = Adam(
    [
        {'params': classifier.parameters()}, 
        {'params': feat_loss.parameters(), 'weight_decay': .0005}
    ],
    lr=.05
)

lr_scheduler = CosineAnnealingLR(optimizer, T_max=N_EPOCHS, eta_min=0.001, last_epoch=-1)
...
for i in range(N_EPOCHS):
    ...
    for j, (trn_imgs_batch, trn_lbls_batch) in enumerate(training_loader):
        ...
        logits_g, logits_v, logits_c, logits_feats = classifier(trn_imgs_batch_device)
        ...
        loss_feat = F.cross_entropy(feat_loss(logits_feats, trn_lbls_batch_device[:, 0]), trn_lbls_batch_device[:, 3], reduction=reduction)
        
        #break
        
        total_trn_loss = .5*loss_g + .25*loss_v + .25*loss_c + .1*loss_feat
        
        total_trn_loss.backward()
        #clip_grad_value_(classifier.parameters(), 1.0)
        #clip_grad_value_(feat_loss.parameters(), 1.0)
    ...
    with torch.no_grad():
        for k, (vld_imgs_batch, vld_lbls_batch) in enumerate(validation_loader):
            ...
            logits_g, logits_v, logits_c, _ = classifier(vld_imgs_batch_device)
