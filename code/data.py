...
class Bengaliai_DS(Dataset):
    ...
    def __init__(self, img_arr, lbl_arr, transform=None, norm=False, split_label=False, RGB=True, raw=True, dtype='float32'):
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            ...
            img = torch.from_numpy(img[np.newaxis, :, :].astype(float))
        
        if self.RGB:
            img = img.repeat(3, 1, 1)
        
        if self.norm:
            img = (img - 0.053022693331160706) / 0.16149050507093382
