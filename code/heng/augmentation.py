...
def do_random_projective(image):
    magnitude=0.4
...
def do_random_perspective(image):
    magnitude=0.4
...
#----
def do_random_grid_distortion(image):
    magnitude=0.4
...
# http://pythology.blogspot.sg/2014/03/interpolation-on-regular-distorted-grid.html
def do_random_custom_distortion1(image):
    magnitude=0.5
...
def do_random_block_fade(image):
    magnitude=0.5
...
def do_random_dilate(image, magnitude=0.54):
    s = int(round(1 + np.random.uniform(0,1)*magnitude*6))
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, tuple((s,s)))
    image  = cv2.dilate(image, kernel, iterations=1)
    return image

def do_random_sprinkle(image):
    magnitude=0.5
